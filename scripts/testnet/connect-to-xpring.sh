...
add_accounts() {
    ...
    local xpring_account_json=$(cat "${ACCOUNT_TEMPLATE_FILE}" | sed \
        ...
        -e "s~<http_endpoint>~${credential_http_endpoint}~g")
    ...
    local our_http_endpoint="$(get_localtunnel_url ${IOH_LT_SUBDOMAIN})/ilp"
    local our_account_json=$(cat "${ACCOUNT_TEMPLATE_FILE}" | sed \
        ...
        -e "s~<http_endpoint>~${our_http_endpoint}~g")
    ...
    echo ${our_account_json} | curl \
        -X POST \
        -H "Authorization: Bearer ${XPRING_ADMIN_AUTH_TOKEN}" \
        -H "Content-Type: application/json" \
        -d @- \
